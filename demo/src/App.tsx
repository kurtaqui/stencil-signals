...
export function App() {
	const [controllerMounted, setControllerMounted] = React.useState(true);
	...
	return (
		<div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: '680px', margin: '2rem auto', padding: '0 1rem' }}>
			...
			<h2 style={h2Style}>
				signal() + computed() — <code>ReactiveControllerHost</code> + <code>SignalWatcherController</code> + <code>watchEffect()</code>
				<button style={destroyBtn} onClick={() => setControllerMounted((m: boolean) => !m)}>
					{controllerMounted ? 'Destroy' : 'Remount'}
				</button>
			</h2>
			<div style={card}>{controllerMounted && <controller-demo />}</div>
